...
export default function AcceptBet() {
  ...
  async function fetchBet() {
    try {
      const { data, error } = await supabase
        .from('bets')
        .select(`*, challenger_profile:profiles!bets_created_by_fkey(id, display_name, avatar_emoji, settle_score)`)
  ...
  async function handleAccept() {
    ...
    try {
      const { error } = await supabase
        .from('bets')
        .update({ challenged_user: user.id, status: 'active', accepted_at: new Date().toISOString() })
        .eq('id', bet.id)
        .eq('status', 'pending')
      if (error) throw error
      navigate(`/bet/${bet.id}`)
  ...
  async function verifyOtp(e) {
    ...
    try {
      ...
      if (error) throw error
      // Session fires → useEffect triggers accept
    } catch (err) {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: 20 }}>
        <div className="card card-top-line" style={{ textAlign: 'center', maxWidth: 400 }}>
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 28, marginBottom: 10, letterSpacing: 1 }}>BET {bet.status.toUpperCase()}</div>
          <div style={{ fontSize: 14, color: 'var(--light)' }}>This bet has already been {bet.status}.</div>
          {session && <button className="btn btn-acid btn-full" style={{ marginTop: 20 }} onClick={() => navigate(`/bet/${bet.id}`)}>View Bet</button>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: 20, minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center', marginBottom: 36 }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 40, letterSpacing: 5 }}>SETTLE<span style={{ color: 'var(--acid)' }}>.</span></div>
      ...
      {bet && (
        <div className="card card-top-line" style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            <div className="avatar" style={{ background: '#2a2a2a' }}>{bet.challenger_profile?.avatar_emoji || '🎲'}</div>
            <div>
              <div style={{ fontWeight: 700, fontSize: 15 }}>{bet.challenger_profile?.display_name}</div>
              <div style={{ fontSize: 10, color: 'var(--acid)', fontFamily: 'var(--font-mono)' }}>SETTLE SCORE: {bet.challenger_profile?.settle_score || 750}</div>
            </div>
            <div style={{ marginLeft: 'auto', fontFamily: 'var(--font-display)', fontSize: 16, color: 'var(--mid)', letterSpacing: 2 }}>CHALLENGES YOU</div>
          </div>
          <div className="bet-statement" style={{ fontSize: 16 }}>"{bet.title}"</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--light)', textTransform: 'capitalize' }}>{bet.resolution_method || 'Mutual'}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {user && user.id !== bet?.created_by && bet?.status === 'pending' && (
        <button className="btn btn-acid btn-full" onClick={handleAccept} disabled={accepting}>
          {accepting ? <span className="spinner spinner-sm" /> : '🤝 ACCEPT BET'}
        </button>
      )}

      {user && user.id === bet?.created_by && (
        <div style={{ textAlign: 'center', padding: 16, background: 'rgba(200,255,0,0.06)', border: '1px solid rgba(200,255,0,0.2)', borderRadius: 'var(--radius-md)' }}>
          <div style={{ fontSize: 14, color: 'var(--acid)' }}>This is your bet! Share the link with your opponent.</div>
          <button className="btn btn-ghost btn-sm" style={{ marginTop: 10 }} onClick={() => navigate(`/bet/${bet.id}`)}>View Bet →</button>
        </div>
      )}

      {!user && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ textAlign: 'center', fontSize: 14, color: 'var(--light)', marginBottom: 8 }}>Sign in to accept this bet</div>
          <div className="card">
            {authStep === 'email' ? (
              <form onSubmit={sendOtp} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div className="form-group">
                  <label className="form-label">Your Email</label>
                  <input className="form-input large" type="email" placeholder="you@example.com"
                    value={email} onChange={e => setEmail(e.target.value)} required autoFocus />
                </div>
                {authError && <div style={{ fontSize: 12, color: 'var(--red)' }}>{authError}</div>}
                <button type="submit" className="btn btn-acid btn-full" disabled={authLoading2 || !email.trim()}>
              ...
              <form onSubmit={verifyOtp} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={{ fontSize: 14, color: 'var(--light)' }}>Code sent to <strong style={{ color: 'var(--white)' }}>{email}</strong></div>
                <div className="form-group">
                  <label className="form-label">6-Digit Code</label>
                  <input className="otp-input" type="number" placeholder="000000"
                    value={otp} onChange={e => setOtp(e.target.value.slice(0, 6))} required autoFocus inputMode="numeric" />
                </div>
                {authError && <div style={{ fontSize: 12, color: 'var(--red)' }}>{authError}</div>}
                ...
                <button type="button" className="btn btn-ghost btn-sm" onClick={() => { setAuthStep('email'); setOtp(''); setAuthError('') }}>
                  ← Change Email
                </button>
              </form>
            )}
          ...
          <div style={{ textAlign: 'center', fontSize: 11, color: 'var(--mid)' }}>
            By accepting you agree to Settle's Terms. Settle is a social record-keeping tool — no money is handled.
          </div>
        </div>
      )}
    </div>
